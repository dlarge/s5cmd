...
// Storage implementation of a local filesystem.
type Filesystem struct {
	dryRun bool
}

// Abstraction of file reading along with metadata about its content type.
type FileReader interface {
	io.ReadCloser
	ContentType() string
}

// Implements FileReader and is a wrapper for os.Stdin to treat it as a local file.
type stdin struct {
	file *os.File
}
...
// Piped read from os.Stdin is plain text.
func (s *stdin) ContentType() string {
	return "text/plain; charset=utf-8"
}

// Implements FileReader and wraps local file with support for content type inference.
